...
void usage()
{
	...
	fprintf(stderr, "Return value: 0, no error; 1, nodes uncolored; 2, edge conflicts;\n");
	fprintf(stderr,
			"             3, color not in list; 4, color set size\n\n");
	...
	while (++i < argc)
	{
		...
		else if (*option == '-')
		{
			...
			case 'h':
				...
				goto error_unknown_long_opt;
			case 'i':
				if (strncmp(option + 1, optstr__instance + 1, option_len - 1) == 0)
				{
					if (argument != 0)
				...
				goto error_unknown_long_opt;
			case 'p':
				if (strncmp(option + 1, optstr__problem + 1, option_len - 1) == 0)
				{
					if (argument != 0)
				...
				goto error_unknown_long_opt;
			case 's':
				if (strncmp(option + 1, optstr__solution + 1, option_len - 1) == 0)
				{
					if (argument != 0)
