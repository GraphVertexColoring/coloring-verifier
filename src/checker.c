...
void checkConstraints_ascii()
{
	...
	while (!feof(col_file))
	{
		...
		else if ((p = strstr((char *)(&buf), "p ")) != NULL)
		{
			...
			if (strstr((char *)&buf, "edge") != NULL || (strstr((char *)&buf, "col") != NULL) || (strstr((char *)&buf, "edges") != NULL) || strstr((char *)&buf, "band") != NULL)
			{
				p++;
				while (*p != '\0' && *p != '\n')
				{
				...
				if (problem == 5)
					for (i = 0; i < solution_sizes[from] - 1; i++)
						for (j = i + 1; j < solution_sizes[to]; j++)
							if (abs(
									solution_sets[from][i] - solution_sets[to][j]) < dist)
							{
								conflict++;
								if (VERBOSE)
									printf("%ld %d %d %d %d %d\n", from, i, j,
			...
			else
			{
				...
				for (i = 0; i < solution_sizes[from]; i++)
					for (j = 0; j < solution_sizes[to]; j++)
						...
						{
							if (solution_sets[from][i] == solution_sets[to][j])
							{
								conflict++;
								if (VERBOSE)
									printf("%ld %ld\n", from, to);
		...
		else if (buf[0] == 'n' && (problem == 3 || problem == 5 || problem == 6))
		{
			tmp = strtok(buf, " ");
			tmp = strtok(0, " ");
			from = atoi(tmp);
			...
			if (problem == 6)
			{
				...
				printf("\n");
				for (i = 0; i < solution_sizes[from] - 1; i++)
					if (solution_sets[from][i + 1] - solution_sets[from][i] != 1)
					{
						conflict++;
						if (VERBOSE)
							printf(
...
unsigned short int checkConstraints()
{
	...
	if (COLORED_NODES == NUM_NODES && CONF_EDGES == 0 && NOT_IN_LIST == 0 && SET_SIZE == 0)
		return 0;
	else if (COLORED_NODES != NUM_NODES)
		return 1;
	else if (CONF_EDGES != 0)
		return 2;
	else if (NOT_IN_LIST != 0)
		return 3;
	else if (SET_SIZE!=0)
		return 4;
}
